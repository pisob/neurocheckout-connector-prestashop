...
class NeuroCheckoutConnectorTelemetryModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    public $auth = false;
    public $ajax = true;
    ...
    public function initContent()
    {
        ...
        if ((int) Configuration::get('NC_TELEMETRY_ENABLED') === 0) {
        ...
        try {
            ...
            $expectedToken = '';
            if ($this->module && method_exists($this->module, 'getTelemetryPublicToken')) {
                $expectedToken = (string) $this->module->getTelemetryPublicToken();
            }
            $providedToken = trim((string) ($payload['token'] ?? $this->headerValue('X-Neuro-Telemetry-Token')));
            unset($payload['token']);
            ...
            $queued = false;
            if ($this->module && method_exists($this->module, 'enqueueTelemetryPayload')) {
                $queued = (bool) $this->module->enqueueTelemetryPayload($payload, 'browser');
            }

            if ($queued && $this->module && method_exists($this->module, 'kickTelemetryDispatch')) {
                $this->module->kickTelemetryDispatch();
            }

            $this->jsonExit(202, true, null, ['queued' => $queued]);
        } catch (\Throwable $e) {
            PrestaShopLogger::addLog('[NC] Telemetry endpoint fatal: ' . $e->getMessage(), 3);
